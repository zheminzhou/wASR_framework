...
class divHMM(object) :
    ...
    def initiate(self, observations, init) :
        ...
        for cutoff in cutoffs :
            ...
            if rec.shape[0] :
                ...
                model['delta'] = np.array([ max(0.00001, min(0.02, (rec[0]+1)/(rec[1]+1) )) for id in np.unique(self.categories['delta']) ])
                model['delta2'] = np.array([ max(0.00001, min(0.02, (rec2[0]+1)/(rec2[1]+1) )) for id in np.unique(self.categories['delta']) ])
    ...
    def estimation(self, model, branch_measures) :
        ...
        for id, delta in enumerate(prediction['delta']) :
            ...
            prediction['delta2'][id] = min(max(prediction['delta2'][id], .00001), .02)

            prediction['delta'][id] = delta_sum[0]/delta_sum[1]
            prediction['delta'][id] = min(max(prediction['delta'][id], .00001), .02)
        ...
        for id, v in enumerate(prediction['v']) :
            ...
            prediction['v'][id] = min(max( prediction['v'][id], (prediction['theta']*prediction['EventFreq'])[0]*2 ), 0.7)
            prediction['v2'][id] = min(max( prediction['v2'][id], (prediction['theta']*prediction['EventFreq'])[0]*2 ), 0.7)
        return prediction
    ...
    def prepare_branches(self, mutations, sequences, missing, interval=None) :
        ...
        for seqId, (seqName, seqLen) in enumerate(sequences) :
            ...
            for m in mutations[(mutations.T[1] == seqId) & (mutations.T[3]>0)].T[2] :
                if s[1,m] < 0 :
