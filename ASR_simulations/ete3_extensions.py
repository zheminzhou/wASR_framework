    ...
    with open(tre_file) as fin:
        for line in fin :
            ...
            if line.lower() == 'translate' :
                ...
                try :
                    tre_name = re.findall(r'tree (\S+) *=', prefix.strip())[0]
                except :
                    tre_name = ''
                tree_line = '(' + tree_line
                annotation_tags = {}
                tree_line = re.sub(r'\[[^\]]+\]',get_tag, tree_line)
                tree_line = re.sub(r':([\d\+\.eE-]+)(___\d+___)', repos, tree_line)
                tre = ete3.Tree(tree_line, format=1)
                for node in tre.traverse() :
                    names = re.findall(r'^(.*)(___\d+___)$', node.name)
                    node.annotations = {}
                    if len(names) :
