...
def read_vcf(vcf_file, ref_file=None):
        ...
        for line in f:
            if line.startswith("##"):
                if current_block!='meta-information':
                    raise (f"Malformed VCF file {vcf_file!r} - all the meta-information (lines starting with ##) must appear at the top of the file.")
                ...
                if not line.startswith(header_start):
                    raise (f"Malformed VCF file {vcf_file!r} - the header line must start with {header_start}")
                if current_block!='meta-information':
                    raise (f"Malformed VCF file {vcf_file!r} - the header must immediately follow the meta-information lines")
                ...
                if current_block!='header' and current_block!='data-lines':
                    raise (f"Malformed VCF file {vcf_file!r} - the data lines must follow the header line")
                ...
                if metadata['chrom'] is None:
                    ...
                    raise (f"The VCF file {vcf_file!r} contains multiple chromosomes. TreeTime can not yet handle this.")
                ...
                if len(calls)!=nsamp:
                    raise (f"Malformed VCF file {vcf_file!r} - the data lines have different number of calls than the number of samples defined in the header")
                ...
                for sname, sa in zip(samps, calls):
                    ...
                    if metadata['ploidy'] is None:
                        ...
                        raise (f"The VCF file {vcf_file!r} had genotype calls of ploidy {metadata['ploidy']} but sample {sname!r} has a genotype of ploidy {ploidy}")
    ...
    if len(positions)==0 and num_insertions==0:
        raise (f"VCF file {vcf_file!r} has no data-lines which we could extract genotype information from!")
